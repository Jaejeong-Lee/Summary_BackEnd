...
// ○ 한 페이지당 10건씩 출력
page_number = 1
db.product.find().sort({num: -1}).skip((page_number - 1) * 10).limit(10)
// ○ 6페이지에 해당하는 문서를 출력하세요.
page_number = 6
db.product.find().sort({num: -1}).skip((page_number - 1) * 10).limit(10)

// product 컬렉션에서 num이 15 미만이거나 19995 초과인 것을 출력하세요.
db.product.find({
    $or: [
        {num: {$lt: 15}},
        {num: {$gt: 19995}}
...
// product 컬렉션에서 name이 '스마트폰10', '스마트폰100', '스마트폰 1000' 중에 하나이면 출력하세요.
db.product.find({
    name: {
        $in: [
            "스마트폰10",
